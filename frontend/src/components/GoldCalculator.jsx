...
export const GoldCalculator = () => {
  ...
  const fetchRates = useCallback(async () => {
    try {
      ...
      setRatesError(false);
      const res = await fetch(`${API_BASE}/api/gold-rate`);
        ...
        <div className="relative z-10 grid items-start grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          <div className="space-y-6 lg:col-span-7">
            <div>
              ...
              <div className="flex items-center gap-2 mb-4">
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-extrabold border transition-all cursor-pointer ${
                    metalType === 'gold'
            ...
            {metalType === 'gold' && (
              <div className="space-y-2.5">
                <label className="block text-xs font-bold tracking-wide uppercase text-slate-300">
                  {text.purityLabel}
                </label>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold tracking-wide uppercase text-slate-300">
                  {text.weightLabel}
                </label>
                {/* Unit Toggle */}
                  ...
                  <div className="flex rounded-lg overflow-hidden border border-slate-800">
                    {['g', 'kg'].map((u) => (
                      <button
                        ...
                        className={`px-3 py-1 text-xs font-extrabold transition-all cursor-pointer ${
                          weightUnit === u
              ...
              <div className="flex items-center justify-between p-4 border bg-slate-900/50 border-slate-800/80 rounded-2xl">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-extrabold text-[#EFE7DB]">{metalType === 'silver' ? 'Silver' : selectedPurity.toUpperCase() + ' Gold'} — Jaipur</span>
                  </div>
                </div>
                <motion.div
          ...
          <div className="h-full lg:col-span-5">
            <div className="flex flex-col justify-between h-full p-6 space-y-6 border bg-slate-900/70 border-slate-800 rounded-2xl">
              <div>
                <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider border-b border-slate-800 pb-3 flex items-center gap-1.5">
              ...
              <div className="pt-4 space-y-4 border-t border-slate-800">
                <div className="flex items-baseline justify-between">
                  ...
                  <motion.span
                    ...
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className={`text-2xl sm:text-3xl font-black tracking-tight price-amount ${
                      metalType === 'silver' ? 'text-slate-300' : 'text-[#D4A75F]'
                    }`}
                  >
